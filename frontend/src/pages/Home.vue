...
const save = async () => {
    if (!form.value.systolic || !form.value.diastolic || !form.value.pulse) return alert(t('home.pleaseFill'));
...
<template>
    <div class="min-h-screen bg-gray-100">
        ...
        <main class="max-w-4xl mx-auto lg:py-3 py-10 px-4 sm:px-6 lg:px-8">
            ...
            <div v-if="authStore.targetUserId" class="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-8">
                <div class="flex">
                    <div class="ml-3">
            ...
            <div class="bg-white rounded-lg shadow-xl p-6 sm:p-10">
                <h1 class="text-3xl font-bold text-gray-900 mb-8 text-center">{{ t('home.newRecord') }}</h1>

                <div class="space-y-6">
                    <div>
                        <label class="block text-xl font-bold text-gray-800 mb-2">{{ t('recordForm.date') }}</label>
                        <input type="datetime-local" v-model="form.recordedAt" autofocus :disabled="isSubmitting"
                    ...
                    <div>
                        <label class="block text-xl font-bold text-gray-800 mb-2">{{ t('recordForm.systolic') }}</label>
                        <input type="number" v-model="form.systolic" :disabled="isSubmitting"
                    ...
                    <div>
                        <label class="block text-xl font-bold text-gray-800 mb-2">{{ t('recordForm.diastolic') }}</label>
                        <input type="number" v-model="form.diastolic" :disabled="isSubmitting"
                            class="mt-1 block w-full border-4 border-indigo-400 rounded-lg shadow-md text-3xl p-5 focus:ring-4 focus:ring-indigo-500 focus:border-indigo-600 bg-white disabled:opacity-50 disabled:cursor-not-allowed"
                            placeholder="80">
                    </div>
                    <div>
                        <label class="block text-xl font-bold text-gray-800 mb-2">{{ t('recordForm.pulse') }}</label>
                        <input type="number" v-model="form.pulse" :disabled="isSubmitting"
                            class="mt-1 block w-full border-4 border-indigo-400 rounded-lg shadow-md text-3xl p-5 focus:ring-4 focus:ring-indigo-500 focus:border-indigo-600 bg-white disabled:opacity-50 disabled:cursor-not-allowed"
                            placeholder="70">
                    </div>
                    ...
                    <div class="pt-6">
                        <button @click="save" :disabled="isSubmitting"
                            ...
                            {{ isSubmitting ? t('home.saving', '儲存中...') : t('home.saveRecord') }}
                        </button>
                    </div>
                </div>
            </div>
