...
const exportCSV = () => {
  ...
  const headers = [t('history.date'), t('history.systolic'), t('history.diastolic'), t('history.pulse')];
...
const handleSave = async (data: any) => {
  try {
    const payload = { ...data, targetUserId: authStore.targetUserId || undefined };
    if (data.id) {
...
<template>
  <div class="min-h-screen bg-gray-100">
    ...
    <main class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
      ...
      <div v-if="authStore.targetUserId" class="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-4">
        <div class="flex">
          <div class="ml-3">
      ...
      <div class="px-4 py-6 sm:px-0">
        <h1 class="text-2xl font-semibold text-gray-900 mb-4">{{ t('history.title') }}</h1>

        <!-- Filter Controls -->
        <div class="mb-4 flex flex-wrap items-end gap-4 bg-white p-4 rounded-lg shadow">
          <div class="flex flex-col">
            <label class="text-xs font-medium text-gray-700 mb-1">{{ t('history.startDate') }}</label>
            <input type="date" v-model="startDate"
              class="border rounded-md px-3 py-2 text-sm focus:ring-indigo-500 focus:border-indigo-500">
          </div>
          <div class="flex flex-col">
            <label class="text-xs font-medium text-gray-700 mb-1">{{ t('history.endDate') }}</label>
            <input type="date" v-model="endDate"
              class="border rounded-md px-3 py-2 text-sm focus:ring-indigo-500 focus:border-indigo-500">
          </div>
          <div class="flex gap-2">
            <button @click="fetchRecords"
              class="bg-indigo-600 text-white px-4 py-2 rounded-md text-sm hover:bg-indigo-700">{{ t('history.filter')
              }}</button>
            <button @click="clearFilter"
              class="bg-gray-200 text-gray-700 px-4 py-2 rounded-md text-sm hover:bg-gray-300">{{ t('history.clear')
              }}</button>
          </div>
          <div class="flex gap-2 ml-auto">
            <button @click="triggerImport"
              class="bg-green-600 text-white px-4 py-2 rounded-md text-sm hover:bg-green-700">{{ t('history.import')
              }}</button>
            <button @click="exportCSV" class="bg-blue-600 text-white px-4 py-2 rounded-md text-sm hover:bg-blue-700">{{
                ...
                <table class="min-w-full divide-y divide-gray-200">
                  <thead class="bg-gray-50">
                    <tr>
                      ...
                      <th scope="col"
                        class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{
                          t('history.systolic') }}</th>
                      <th scope="col"
                        class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{
                          t('history.diastolic') }}</th>
                      <th scope="col"
                        class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{
                          t('history.pulse') }}</th>
                      <th scope="col"
                        class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{
                          t('history.actions') }}</th>
                    </tr>
                  </thead>
                  <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-for="record in records" :key="record.id">
                      ...
                      <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 space-x-2">
                        <button @click="openEditModal(record)" class="text-indigo-600 hover:text-indigo-900">{{
    ...
    <RecordModal :isOpen="showModal" :initialData="editingRecord" @close="showModal = false" @save="handleSave" />
  </div>
</template>
